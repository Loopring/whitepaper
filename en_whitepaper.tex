...
In order to encourage an exchange to offer best rate for users, Loopring would distribute profit from \texttt{margin} to the given exchange. as an order $O^i$,  if price for purchasing is $b^i$( $b^i \le \overline{b^i}$ ),  then we define margin as: 
...
In order to encourage $LRC$ usage, if the order has no preset token fee $m^i$, or $m^i=0$, then the actual ratio is 100\% regardless of the relevant hash in this order. If none of the order has set up this rate $\Theta=100\%$, then all proceeds from the margin will go to the exchange.
