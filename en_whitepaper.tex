...
\subsubsection{Sub-Ring Checking}
...
%		text width=5cm,
 ...
 \draw [draw,->] (order1) -- node [label, xshift=-2cm] {\textbf{7898 tokenA}} (order5);
 \draw [draw,->] (order2) -| node [label, xshift=-1.6cm] {\textbf{8 tokenB}} (order1);
 \draw [draw,->] (order3) |- node [label, xshift=1cm, yshift=0.24cm] {\textbf{98 tokenC}} (order2);
 \draw [draw,->] (order4) -- node [label, xshift=1.8cm] {\textbf{98 tokenC}} (order3);
 \draw [draw,->] (order5) -- node [label, yshift=0.2cm] {\textbf{98 tokenC}} (order4);
...
scale=0.75,
font=\bfseries\footnotesize\sffamily,
force/.style={rectangle, draw, text badly centered, minimum height=0.6cm}] 

% Draw forces
\node [force] (total) {Total Buy Amount};
\node [force, above of=total] (origin) {Original Buy Amount};
\node [force, fill=black!10, right of=origin] (margin) {Margin};
\node [force, above of=origin] (user) {Final User Buy Amount};
\node [force, fill=black!20, right of=user] (split) {Margin Split};
...
\section{Protocol Ethereum Smart-Contracts}
