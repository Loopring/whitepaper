...
\begin{itemize}
	\item Make fee payment more flexible: remove the friction of requiring a specific fee-payment token.
...
<DEFINITION>
P2P order: If the order’s tokenSFeePercentageor tokenBFeePercentageis greater than zero, the order is a P2P order. Other fee parameters in the P2P order will be ignored by the protocol.

Normal Order Parameters:

\begin{itemize}
	\item address feeToken: optional and defaults to LRC token address if not specified. This is the type of ERC20 token to be paid to the miner as matching fees.
	\item unit feeAmount: optional and defaults to 0. This is the amount of feeToken to be paid. This number should be calculated by the wallet software off-chain.
	\item uint16 feePercentage: optional and defaults to 0. This is the percentage of tokenB (token that user is buying) to be paid as matching fees if the order does not have enough feeToken to pay. This parameter should also be supplied by the wallet.
	\item uint16 waiveFeePercentage: optional and defaults to 0. If this number is positive, the miner will waive a waiveFeePercentage\% percentage of the fees before the burn; if this number is negative, the miner will reward-waiveFeePercentage\%of its total fee after the burn to the order — the reward is also subject to burn.
\end{itemize}

P2P Order Parameters:
\begin{itemize}
	\item tokenSFeePercentage: optional and defaults to 0. The percentage of tokenS to charge by the wallet.
	\item tokenBFeePercentage: optional and defaults to 0. The percentage of tokenB to charge by the wallet.
\end{itemize}
...
However, if the wallets corresponding to the two P2P orders are set to charge different tokenSFeePercentageor tokenBFeePercentage, the actual fee and transaction calculation is more complicated:
...
\begin{enumerate}
	\item Pay the lowest fees for their transactions,
	\item Benefit from all network-wide activity.
\end{enumerate}
...
\begin{enumerate}
	\item Increasing LRC demand,
	\item Reducing LRC supply.
\end{enumerate}

\subsection{Fee Rebate}
Loopring 2.0 also allows LRC token holders to lock up LRC in a smart contract in exchange for a percentage of fees to be burned otherwise, or a rebate, for each and every order during the lockup period.

For example, if in an order the total fee is 10 AnyToken, and 4 AnyToken is to be burned. Assuming the fee payer locked up 1 million LRC and has a 50\% rebate rate. He/she will ended up with paying only 8 AnyToken and 2 of them will be burned. The rebate comes from fees otherwise to be burned, not payee’s income. Please also note that payee’s rebate rate doesn’t apply.

A rebate rate of 100\% ensures all orders are burn-free. The Fee-Rebate feature will be very helpful for high frequent traders.
